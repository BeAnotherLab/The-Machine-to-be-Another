...
vec4 hsv2rgb(vec4 c)
{
    ...
    vec3 col = c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
    return vec4(col, 1.0);
}
...
void main()
{
    ...
    */
     ...
     vec4  colL = rgb2hsv(texture2DRect(tex,pos + shiftL));
     vec4  colA = rgb2hsv(texture2DRect(tex,pos + shiftA));
     vec4  colB = rgb2hsv(texture2DRect(tex,pos + shiftB));
